...
import           Pos.Core (Address (..), Coin, Currency, HasConfiguration, IsBootstrapEraAddr (..),
                           makePubKeyAddress)
...
class Monad m => MonadBalances m where
    ...
    getBalance :: Address -> m (NonEmpty (Currency, Coin))
...
getBalanceFromUtxo :: MonadBalances m => Address -> m (NonEmpty (Currency, Coin))
