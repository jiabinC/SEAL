...
-- !!! 注意: 这个文件中中出现的Utxo都已经过滤为只剩下Seal币交易 !!!
...
import           Pos.Core (Address, Coin, Currency, StakeholderId, TxFeePolicy (..), TxSizeLinear (..),
                           bvdTxFeePolicy, calculateTxSizeLinear, coinToInteger, integerToCoin,
                           isRedeemAddress, mkCoin, sumCoins, txSizeLinearMinValue,
                           unsafeIntegerToCoin, unsafeSubCoin, sealCurrency)
...
makeMPubKeyTxAddrs pm hdwSigners = makeMPubKeyTx pm getSigner
  where
    getSigner = hdwSigners . txOutAddress
    ...
    pickInputs inps = do
        ...
        if moneyLeft == mkCoin 0
            ...
            case mNextOut of
                ...
                Just inp@(_, TxOutAux{..}) -> do
                    ipsMoneyLeft .= unsafeSubCoin moneyLeft (min (txOutValue toaOut) moneyLeft)
...
mkOutputsWithRem addrData TxRaw {..} currency
    | trRemainingMoney == mkCoin 0 = pure trOutputs
    | otherwise = do
        changeAddr <- lift . lift $ getNewAddress addrData
        let txOut = TxOut changeAddr currency trRemainingMoney
...
createFakeTxFromRawTx pm fakeAddr TxRaw{..} =
    let fakeOutMB
            ...
            | otherwise =
                Just $
                TxOutAux (TxOut fakeAddr sealCurrency trRemainingMoney)
