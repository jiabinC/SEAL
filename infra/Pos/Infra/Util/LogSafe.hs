...
import           Pos.Core.Txp (UserCert)
import           Pos.Core.Common (Address, Coin, Currency)
...
instance Buildable (SecureLog Currency) where
    build _ = "<currency>"
...
instance Buildable (SecureLog UserCert) where
    build _ = "<certificate(s)>"
