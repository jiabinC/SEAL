...
import           Pos.Core.Common (Address (..), Currency (..), Coeff (..), Coin (..), CoinPortion (..), SharedSeed (..),
                                  StakeholderId, TxFeePolicy (..), TxSizeLinear (..), addressF,
                                  decodeTextAddress, getCoinPortion, unsafeGetCoin, mkCurrency)
...
instance Monad m => ToJSON m Currency where
    toJSON = pure . JSString . show . getCurrency
...
instance Monad m => ToJSON m (Address, Currency, Coin) where
    toJSON (addr, currency, coin) = do 
        jaddr <- toJSON addr
        jcurrency <- toJSON currency
        jcoin <- toJSON coin
        return $ JSArray [jaddr, jcurrency, jcoin]
...
instance ReportSchemaErrors m => FromJSON m Currency where
    fromJSON val = do
        i <- tryParseString readUnsignedDecimal val
        return $ mkCurrency i
...
instance ReportSchemaErrors m => FromJSON m (Address, Currency, Coin) where
    fromJSON val = 
        case val of
        JSArray record -> do
            addr <- fromJSON $ record!!0
            currency <- fromJSON $ record!!1
            amount <- fromJSON $ record!!2
            return (addr, currency, amount)
        _           -> expectedButGotValue "Array" val
