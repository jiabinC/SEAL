...
import           Pos.Core.Common (Address, Currency (..), Coin, StakeholderId, unsafeGetCoin)
...
instance ToJSON GenesisNonAvvmBalances where
    toJSON = toJSON . convert . getGenesisNonAvvmBalances
      where
        convert :: [(Address, Currency, Coin)] -> [(Text, Integer, Integer)]
        convert = map f
        f :: (Address, Currency, Coin) -> (Text, Integer, Integer)
        f (addr, currency, amount) = (pretty addr, toInteger $ getCurrency currency, toInteger $ unsafeGetCoin amount)
