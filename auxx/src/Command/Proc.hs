...
import           Pos.Core (Coin, Currency, AddrStakeDistribution (..), Address, HeavyDlgIndex (..),
                           SoftwareVersion (..), StakeholderId, addressHash, mkMultiKeyDistr, mkCoin,
                           unsafeGetCoin, sealCurrency)
...
createCommandProcs mpm hasAuxxMode printAction mDiffusion = rights . fix $ \commands -> [
    ...
    , cpArgumentConsumer = do
        ...
        let txOutCurrency = sealCurrency
    ...
    , cpExec = \addr' -> do
        ...
        return $ ValueNumber (fromIntegral . unsafeGetCoin $ sealBalance balance)
  ...
  where
    ...
    sealBalance :: (NonEmpty (Currency, Coin)) -> Coin
    sealBalance balances = HM.lookupDefault (mkCoin 0) sealCurrency $ HM.fromList $ NE.toList balances
