...
import           Pos.Core.Txp (Tx (..), TxAux (..), TxId, TxOut (..), TxOutAux (..), TxUndo, pattern SealTxOut)
...
getBalanceUpdate txAux txUndo =
    let f txOuts = [(addr, coin)| SealTxOut addr coin <- txOuts]
        minusBalance = f $ map toaOut $ catMaybes $ toList txUndo
        plusBalance = f $ toList $ _txOutputs (taTx txAux)
    in BalanceUpdate {..}
