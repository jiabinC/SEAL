...
import           Pos.Core (HasConfiguration, addressHash, checkPubKeyAddress,
                           ...
                           mkCoin, sumCoins, withGenesisSpec, sealCurrency)
...
scriptTxSpec pm = describe "script transactions" $ do
  ...
  where
    ...
    randomPkOutput = runGen $ do
        key <- arbitrary
        return (TxOut (makePubKeyAddressBoot key) sealCurrency (mkCoin 1))
    ...
    checkScriptTx val mkWit =
        let (inp, _, utxo) = mkUtxo $
                TxOut (makeScriptAddress Nothing val) sealCurrency (mkCoin 1)
