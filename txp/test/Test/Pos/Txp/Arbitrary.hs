...
import           Pos.Core.Txp (Tx (..), TxAux (..), TxIn (..), TxInWitness (..), TxOut (..),
                               ...
                               mkTxPayload, UserCert (..), pattern SealTxOut, SealCert (..), SealState (..))
...
-- Arbitrary txp types
----------------------------------------------------------------------------
instance Arbitrary UserCert where
    arbitrary = genericArbitrary

instance Arbitrary SealCert where
    arbitrary = genericArbitrary

instance Arbitrary SealState where
    arbitrary = genericArbitrary
...
buildProperTx pm inputList (inCoin, outCoin) =
  ...
  where
    ...
    makeTxOutput s c =
        SealTxOut (makePubKeyAddress (IsBootstrapEraAddr True) $ toPublic s) c
