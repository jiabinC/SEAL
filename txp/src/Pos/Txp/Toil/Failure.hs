...
data ToilVerFailure
    ...
    | ToilInvalidInput  !Word32 !TxIn !TxOut
    | ToilMissingInputInSeal -- when inputs does not contains TxOut in seal(currency)
    | ToilInvalidCurrencyGroup -- when inputs's currency group does not match outputs's currency group
    | ToilOutNotEqualIn !Integer !Integer -- used while verify tx of currencies that is not seal currency  
...
instance Buildable ToilVerFailure where
    ...
    build (ToilOutGreaterThanIn tInputSum tOutputSum) =
        bprint ("sum of outputs is greater than sum of inputs ("%int%" < "%int%") for seal")
    ...
    build (ToilInvalidOutput n reason) =
        bprint (ords%" output is invalid:\n'"%
                " reason: "%build)
            n reason
    build (ToilInvalidInput n txIn txOut) = 
        bprint ("input /output type doesn't match "%build%
                "  input: "%build%"\n"%
                "  output: "%build)
            n txIn txOut
    build ToilMissingInputInSeal =
        "missing inputs in seal coin"
    build ToilInvalidCurrencyGroup =
        "invalid currency group"
    build (ToilOutNotEqualIn tInputSum tOutputSum) =
        bprint ("sum of outputs is not equal sum of inputs ("%int%" != "%int%") for non seal")
        tInputSum tOutputSum
