...
import           Pos.Core (AddrType (..), Address (..), isRedeemAddress,
...
import           Pos.Txp.Toil.Utxo.Verify (runVerifyM, verifyTx)
    ...
    case (vtcVerifyAllIsKnown, unknownTxInMB) of
        ...
        (_, Nothing) -> do
            ...
            liftEither $ do
                ...
                txFee <- runVerifyM resolvedInputs _txOutputs verifyTx
...
verifySums ::
    ...
    -> Either ToilVerFailure TxFee
verifySums resolvedInputs outputs 
    | sealCurrency `notElem` inputCnys = throwError ToilMissingInputInSeal
    | not $ inputCnysExclSeal == outputCnysExclSeal = throwError ToilInvalidCurrencyGroup    
    | otherwise = do
        txFee <- verifySumOfSeal resolvedInputs outputs
        mapM_ (verifySumOfOtherCny resolvedInputs outputs) inputCnysExclSeal
        return txFee
    where  
        inputCnys = NE.sort $ NE.nub $ map (txOutCurrency . toaOut . snd) resolvedInputs
        outputCnys = NE.sort $ NE.nub $ map txOutCurrency outputs
        inputCnysExclSeal = NE.filter (not . (==) sealCurrency) inputCnys -- currencies without seal coin
        outputCnysExclSeal = NE.filter (not . (==) sealCurrency) outputCnys -- currencies without seal coin

verifySumOfSeal ::
       NonEmpty (TxIn, TxOutAux)
    -> NonEmpty TxOut
    -> Either ToilVerFailure TxFee
verifySumOfSeal resolvedInputs outputs =
  case mTxFee of
  ...
  where
    mTxFee = TxFee <$> rightToMaybe (integerToCoin (inpSum - outSum))
    outSum = sumCoins $ map txOutValue $ NE.filter (matchCurrency sealCurrency) outputs 
    inpSum = sumCoins $ map txOutValue $ NE.filter (matchCurrency sealCurrency) $ map (toaOut . snd) resolvedInputs
...
verifySumOfOtherCny resolvedInputs outputs cny 
    | isSumEquals = return () 
    | otherwise = throwError $ ToilOutNotEqualIn inpSum outSum
  where
    isSumEquals = outSum == inpSum
    outSum = sumCoins $ map txOutValue $ NE.filter (matchCurrency cny) outputs 
    inpSum = sumCoins $ map txOutValue $ NE.filter (matchCurrency cny) $ map (toaOut . snd) resolvedInputs
--}
...
verifyKnownInputs protocolMagic VTxContext {..} resolvedInputs TxAux {..} = do
  ...
  where
    ...
    checkInput
        ...
        -> TxInWitness
        -> Either ToilVerFailure ()
    checkInput i (txIn, toa@(TxOutAux {..})) witness = do
        unless (checkSpendingData (txOutAddress toaOut) witness) $
            throwError $ ToilWitnessDoesntMatch i txIn toaOut witness
        whenLeft (checkWitness toa witness) $ \err ->
            throwError $ ToilInvalidWitness i witness err
        case (txIn, toaOut) of
            (TxInUtxo {..}, TxOut {..}) -> return ()
            (TxInUserCert {..}, TxOutUserCert {..}) -> return ()
            _ -> throwError $ ToilInvalidInput i txIn toaOut
