...
import           Pos.Txp (Tx (..), TxId, TxIn (..), TxOut (..), TxOutAux (..), pattern SealTxOut)
...
generateRealTxHistE outputAddresses = do
    ...
    let genTxOut :: Gen [TxOut]
        genTxOut = forM outputAddresses $ \address -> SealTxOut address <$> genCoins
...
generateFakeUtxo RangeDistribution{..} aId = do
    ...
    let txsOut :: [TxOutAux]
        txsOut = map (\address -> TxOutAux $ SealTxOut address coinAmount) generatedAddresses
