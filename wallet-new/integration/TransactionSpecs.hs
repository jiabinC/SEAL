...
transactionSpecs wRef wc = do
    let sealAmount acct = caAmount $ Unsafe.head $ NE.filter (\ca -> (caCurrency ca) == (V1 Core.sealCurrency)) $ accAmount acct -- accAmount contains seal currency
        ...
        it "posted transactions appear in the index" $ do
            ...
            let payment = Payment
                    ...
                    , pmtDestinations = pure PaymentDistribution
                        ...
                        , pdCurrency = V1 Core.sealCurrency
                        , pdAmount = tenthOf (sealAmount fromAcct)
                        }
        ...
        it "fails if you spend too much money" $ do
            ...
            let payment = Payment
                    ...
                    , pmtDestinations = pure PaymentDistribution
                        { pdAddress = addrId toAddr
                        , pdCurrency = V1 Core.sealCurrency
                        , pdAmount = tooMuchCash (sealAmount toAcct)
                        }
                    , pmtGroupingPolicy = Nothing
                    , pmtSpendingPassword = Nothing
                    }
