...
import           Pos.Core (getCoin, mkCoin, unsafeAddCoin, unsafeSubCoin, sealCurrency)
        ...
        PostWallet -> do
            ...
            case eresult of
                Right WalletResponse { wrData = result } -> do
                    let balance = walBalance result
                    checkInvariant
                        ((NE.length balance) == 1 && (unV1 . caAmount $ NE.head balance) == minBound)
...
-- Addresses
        ...
        -- Transactions
        PostTransaction -> do
            let sealAmount acct = caAmount $ Unsafe.head $ NE.filter (\ca -> (caCurrency ca) == (V1 sealCurrency)) $ accAmount acct -- accAmount contains seal currency
            localAccounts  <- use accounts
            ...
            let localAccsWithMoney = filter ((> minCoinForTxs) . sealAmount) localAccsNotLocked
            ...
            let accountSourceMoney = sealAmount accountSource
                reasonableFee = 100
            ...
            let _expectedNewBalance =
                    V1 $
                        (unV1 (sealAmount accountSource) `unsafeSubCoin` moneyAmount)
            ...
            let _expectedDestinationBalance =
                    V1 (unV1 (sealAmount accountDestination)
