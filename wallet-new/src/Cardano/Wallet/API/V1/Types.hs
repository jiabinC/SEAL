...
instance ToJSON (V1 Core.Currency) where
    toJSON (V1 c) = toJSON . Core.getCurrency $ c

instance FromJSON (V1 Core.Currency) where
    parseJSON v = do
        i <- Core.Currency <$> parseJSON v
        either (fail . toString) (const (pure (V1 i)))
            $ Core.checkCurrency i

instance Arbitrary (V1 Core.Currency) where
    arbitrary = fmap V1 arbitrary

instance ToSchema (V1 Core.Currency) where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "V1Currency") $ mempty
            & type_ .~ SwaggerNumber
            & minimum_ .~ Just (fromIntegral Core.minCurrencyVal)

instance Hashable (V1 Core.Currency) where
    hashWithSalt i (V1 c) = hashWithSalt i (Core.getCurrency c)

instance Arbitrary (V1 Core.UserCert) where
    arbitrary = fmap V1 $ Core.UserCert
                       <$> arbitrary
                       <*> arbitrary
...
instance ToSchema (V1 Core.UserCert) where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "V1Certificate") $ mempty
            & type_ .~ SwaggerString
...
data CurrencyAmount = CurrencyAmount {
    caCurrency :: (V1 Core.Currency),
    caAmount   :: (V1 Core.Coin)
} deriving (Eq, Show, Ord)

instance Buildable CurrencyAmount where
    build CurrencyAmount{..} = bprint ("{"
        %" currency="%build
        %" amount="%build
        %" }")
        caCurrency
...
instance Buildable (SecureLog CurrencyAmount) where
    build _ = "<currency_amount>"

instance ToJSON CurrencyAmount where  
    toJSON CurrencyAmount{..} = object [ "currency" .= (toJSON $ Core.getCurrency $ unV1 caCurrency)
                                       , "amount"   .= (toJSON $ Core.getCoin $ unV1 caAmount)
                                       ]

instance FromJSON CurrencyAmount where  
    parseJSON (Object v) = CurrencyAmount
        <$> v .: "currency"
        <*> v .: "amount"
    parseJSON x = typeMismatch "parseJSON failed for BackupPhrase" x

instance ToSchema CurrencyAmount where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "CurrencyAmount") $ mempty
            -- & type_ .~ SwaggerString

instance Arbitrary CurrencyAmount where
  arbitrary = CurrencyAmount <$> arbitrary
                             <*> arbitrary

type BalanceAmount = NonEmpty CurrencyAmount

instance Buildable BalanceAmount where
    build balanceAmount = 
        bprint (""%listJson%"") $ NE.toList balanceAmount

instance Buildable (SecureLog BalanceAmount) where
    build _ = "<balances>"
...
instance ToSchema PaymentDistribution where
  declareNamedSchema =
    genericSchemaDroppingPrefix "pd" (\(--^) props -> props
      ...
      & ("currency" --^ "Currency of coin, in number.")
...
instance ToJSON (V1 Core.UserCert) where
    toJSON (V1 c) = toJSON c

instance FromJSON (V1 Core.UserCert) where
    parseJSON v = fmap V1 $ parseJSON v

--  CertDistribution
data CertDistribution = CertDistribution {
      cdAddress  :: !(V1 Core.Address)
    , cdCert     :: !(V1 Core.UserCert)
    } deriving (Show, Ord, Eq, Generic)

deriveJSON Serokell.defaultOptions ''CertDistribution

instance ToSchema CertDistribution where
  declareNamedSchema =
    genericSchemaDroppingPrefix "cd" (\(--^) props -> props
      & ("address" --^ "Address to map certificates to.")
      & ("certificate" --^ "Certificate.")
    )

instance Arbitrary CertDistribution where
  arbitrary = CertDistribution <$> arbitrary
                               <*> arbitrary

deriveSafeBuildable ''CertDistribution
instance BuildableSafeGen CertDistribution where
    buildSafeGen sl CertDistribution{..} = bprint ("{"
        %" address="%buildSafe sl
        %" certificate="%buildSafe sl
        %" }")
        cdAddress
        cdCert
...
data Transaction = Transaction
  ...
  , txAmount        :: !BalanceAmount
  , txInputs        :: !(NonEmpty PaymentDistribution)
  , txOutputs       :: !(NonEmpty PaymentDistribution)
  , txCertInputs    :: !(NonEmpty CertDistribution)
  , txCertOutputs   :: !(NonEmpty CertDistribution)
...
instance ToSchema Transaction where
  declareNamedSchema =
    genericSchemaDroppingPrefix "tx" (\(--^) props -> props
      ...
      & ("certInputs"    --^ "One or more input certificate distributions.")
      & ("certOutputs"   --^ "One or more ouputs certificate distributions.")
...
instance BuildableSafeGen Transaction where
    buildSafeGen sl Transaction{..} = bprint ("{"
        ...
        %" certInputs="%buildSafeList sl
        %" certOutputs="%buildSafeList sl
        %" type="%buildSafe sl
