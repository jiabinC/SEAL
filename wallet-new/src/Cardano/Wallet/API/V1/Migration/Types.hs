...
instance Migrate from to => Migrate (NonEmpty from) (NonEmpty to) where
    eitherMigrate = mapM eitherMigrate
...
instance Migrate V0.CCurrency (V1 Core.Currency) where
    eitherMigrate c =
        let err = Left . Errors.MigrationFailed . mappend "error migrating V0.CCurrency -> Core.Currency, failed: "
        in either err (pure . V1) (V0.decodeCType c)

instance Migrate (V1 Core.Currency) V0.CCurrency where
    eitherMigrate (V1 c) = pure (V0.encodeCType c)

instance Migrate V0.CurrencyAmount V1.CurrencyAmount where
    eitherMigrate c = do
        let err = Left . Errors.MigrationFailed . mappend "error migrating V0.CurrencyAmount -> (V1.CurrencyAmount) : "
        case (V0.decodeCType c) of
            Left err'     -> err err'
            Right pair -> Right $ V1.CurrencyAmount (V1 $ fst pair) (V1 $ snd pair)

instance Migrate V1.CurrencyAmount V0.CurrencyAmount where
    eitherMigrate V1.CurrencyAmount{..} = pure (V0.encodeCType ((V1.unV1 caCurrency), (V1.unV1 caAmount)))
...
instance Migrate (V0.CId V0.Addr, V0.CCurrency, V0.CCoin) V1.PaymentDistribution where
    eitherMigrate (cIdAddr, cCurrency, cCoin) = do
        pdAddress  <- eitherMigrate cIdAddr
        pdCurrency <- eitherMigrate cCurrency
...
instance Migrate (V0.CId V0.Addr, Core.Coin) V1.PaymentDistribution where
    eitherMigrate (cIdAddr, coin) = do
        pdAddress <- eitherMigrate cIdAddr
        pure $ V1.PaymentDistribution pdAddress (V1 Core.sealCurrency) (V1 coin)
...
instance Migrate V1.PaymentDistribution (V0.CId V0.Addr, Core.Currency, Core.Coin) where
    eitherMigrate V1.PaymentDistribution {..} =
        let (V1 amount) = pdAmount
            (V1 currency) = pdCurrency
            (V1 addr)   = pdAddress
            in pure (V0.encodeCType addr, currency, amount)

instance Migrate (V0.CId V0.Addr, Core.Currency, Core.Coin) V1.PaymentDistribution where
    eitherMigrate (cIdAddr, currency, coin) = do
        pdAddress <- eitherMigrate cIdAddr
        pure $ V1.PaymentDistribution pdAddress (V1 currency) (V1 coin)

instance Migrate V0.CUserCert (V1 Core.UserCert) where
    eitherMigrate c =
        let err = Left . Errors.MigrationFailed . mappend "error migrating V0.CUserCert -> Core.UserCert, failed: "
        in either err (pure . V1) (V0.decodeCType c)

instance Migrate (V1 Core.UserCert) V0.CUserCert where
    eitherMigrate (V1 c) = pure (V0.encodeCType c)

instance Migrate (V0.CId V0.Addr, V0.CUserCert) V1.CertDistribution where
    eitherMigrate (cIdAddr, cUserCert) = do
        cdAddress  <- eitherMigrate cIdAddr
        cdCert <- eitherMigrate cUserCert
        pure $ V1.CertDistribution {..}
...
instance Migrate V0.CTx V1.Transaction where
    eitherMigrate V0.CTx{..} = do
        ...
        txCertInputs  <- eitherMigrate ctCertInputs
        txCertOutputs <- eitherMigrate ctCertOutputs
