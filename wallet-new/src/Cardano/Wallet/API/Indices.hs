...
-- instance ToIndex Wallet (NonEmpty BalanceAmount) where
...
-- type WalletIxs        = '[WalletId, BalanceAmount, V1 Core.Timestamp]
type WalletIxs        = '[WalletId,V1 Core.Timestamp]
...
instance Indexable WalletIxs Wallet where
  indices = ixList (ixFun (\Wallet{..} -> [walId]))
                --    (ixFun (\Wallet{..} -> let balance = walBalance in [balance]))
