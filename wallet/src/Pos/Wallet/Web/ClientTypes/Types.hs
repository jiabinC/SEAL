...
import           Pos.Core (BlockVersion, ChainDifficulty, Coin, Currency (..), ScriptVersion, SoftwareVersion,
                           unsafeGetCoin)
...
newtype CCurrency = CCurrency
    { getCCurrency :: Text
    } deriving (Show, Eq, Generic, Buildable, NFData)

mkCCurrency :: Currency -> CCurrency
mkCCurrency = CCurrency . show . getCurrency

newtype CUserCert = CUserCert
    { getCUserCert :: Text
    } deriving (Show, Eq, Generic, Buildable, NFData)
...
instance Buildable CurrencyAmount where
    build CurrencyAmount{..} =
        bprint ("{ currency="%build
                %" amount="%build
                %" }")
        currency
        amount

mkCurrencyAmount :: (Currency, Coin) -> CurrencyAmount
mkCurrencyAmount (currency, amount) = CurrencyAmount (mkCCurrency currency) (mkCCoin amount)

instance Buildable (NonEmpty CurrencyAmount) where
    build cas = foldl (<>) (fromString "") $ map show cas -- TODO xiaolie
...
data CWallet = CWallet
    ...
    , cwAmount         :: !(NonEmpty CurrencyAmount)
...
data CAccount = CAccount
    ...
    , caAmount    :: !(NonEmpty CurrencyAmount)
    } deriving (Show, Generic, Typeable)
...
data CAddress = CAddress
    { cadId       :: !(CId Addr)
    , cadAmount   :: !(NonEmpty CurrencyAmount)
    , cadIsUsed   :: !Bool
...
data CTx = CTx
    { ctId            :: CTxId
    , ctAmount        :: NonEmpty CurrencyAmount
    , ctConfirmations :: Word
    , ctMeta          :: CTxMeta
    , ctInputs        :: [(CId Addr, CCurrency, CCoin)]
    , ctOutputs       :: [(CId Addr, CCurrency, CCoin)]
    , ctCertInputs    :: [(CId Addr, CUserCert)]
    , ctCertOutputs   :: [(CId Addr, CUserCert)]
    , ctIsLocal       :: Bool
    , ctIsOutgoing    :: Bool
...
instance Buildable CTx where
    build CTx{..} =
        ...
        (buildTxEndsCert ctCertInputs)
        (buildTxEndsCert ctCertOutputs)
        ctIsLocal
        ctIsOutgoing
        ctCondition
      where
        buildTxEnds =
            mconcat . intersperse ", " .
            map (\(addr, cny, coin) -> bprint (build%" - "%build%" - "%build) addr cny coin)
        buildTxEndsCert =
            mconcat . intersperse ", " .
...
data NewBatchPaymentMulti = NewBatchPaymentMulti
    { npbmFrom                 :: CAccountId
    , npbmTo                   :: NonEmpty (CId Addr, Currency, Coin)
    , npbmInputSelectionPolicy :: InputSelectionPolicy
    } deriving (Show, Generic)

instance BuildableSafeGen NewBatchPaymentMulti where
    buildSafeGen sl NewBatchPaymentMulti {..} =
        bprint ("{ from="%secretOnlyF sl build
                -- %" to="%secureListF sl (later tripleBuilder) TODO xiaolie 
                %" inputSelectionPolicy="%secretOnlyF sl build
                %" }")
        npbmFrom
        -- npbmTo
...
deriveSafeBuildable ''NewBatchPaymentMulti

data NewCert = NewCert
    { ncInputs          :: [(CTxId, Word32)]
    , ncOutputs         :: NonEmpty (Text, Text)
    } deriving (Show, Generic)

instance BuildableSafeGen NewCert where
    buildSafeGen sl NewCert {..} =
        bprint ("{ in = "%secretOnlyF sl (later mapBuilder)
                %" out = "%secretOnlyF sl (later mapBuilder)
                %" }")
        ncInputs ncOutputs
