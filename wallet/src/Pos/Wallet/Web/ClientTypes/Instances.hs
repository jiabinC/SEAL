...
import           Pos.Core (Address, Coin (..), Currency (..), UserCert (..), checkCoin, checkCurrency, 
                           coinToInteger, decodeTextAddress, mkCoin, unsafeGetCoin, mkCurrency, sealCurrency)
...
import           Pos.Wallet.Web.ClientTypes.Types (AccountId (..), CAccount (..), CAccountId (..),
                                                   CAddress (..), CCoin (..), CCurrency (..), CurrencyAmount (..),
                                                   CUserCert (..), CElectronCrashReport (..), CHash (..), CId (..),
                                                   CPassPhrase (..), CPtxCondition (..), CTx (..),
                                                   CTxId (..), CWallet (..), ScrollLimit (..),
                                                   ScrollOffset (..), mkCTxId, mkCurrencyAmount)
...
type instance OriginType CCurrency = Currency

instance FromCType CCurrency where
    decodeCType =
        maybe (Left "Invalid currency number") Right .
        fmap mkCurrency . readMaybe . toString . getCCurrency

instance ToCType CCurrency where
    encodeCType = CCurrency . show . getCurrency

type instance OriginType CurrencyAmount = (Currency, Coin)

instance FromCType CurrencyAmount where
    decodeCType CurrencyAmount{..} = do
        currency' <- decodeCType currency
        coin <- decodeCType amount
        return (currency', coin)

instance ToCType CurrencyAmount where
    encodeCType (currency, coin) = 
        let ccurrency = encodeCType currency
            ccoin = encodeCType coin
        in CurrencyAmount { currency=ccurrency, amount=ccoin}

type instance OriginType CUserCert = UserCert

deriving instance Read UserCert

instance FromCType CUserCert where
    decodeCType =
        maybe (Left "Invalid certificate data") Right .
        readMaybe . toString . getCUserCert

instance ToCType CUserCert where
    encodeCType = CUserCert . show
...
instance ToHttpApiData Currency where
    toQueryParam = pretty . getCurrency

instance FromHttpApiData Currency where
    parseUrlPiece p = do
        c <- Currency <$> parseQueryParam p
        checkCurrency c
        pure c
...
instance HasTruncateLogPolicy CAddress where
    ...
    truncateLogPolicy addrs =
      ...
      where
        zeroMoney = fromList [mkCurrencyAmount (sealCurrency, mkCoin 0)]
