...
import           Pos.Txp (AddrCoinMap, Utxo, UtxoModifier, 
                          applyUtxoModToCurrencyAddrCoinMapMap, utxoToCurrencyAddrCoinMap)
...
type WalletBalances = HashMap Currency AddrCoinMap
...
updateWalletBalancesAndUtxo modifier = do
    (walletBalances, utxo) <- (,) <$> use wsBalances <*> use wsUtxo
    updateWalletBalances modifier utxo walletBalances 
    wsUtxo %= MM.modifyMap modifier

updateWalletBalances :: UtxoModifier -> Utxo -> WalletBalances -> Update () 
updateWalletBalances modifier utxo walletBalances = do
        wsBalances .= applyUtxoModToCurrencyAddrCoinMapMap modifier utxo walletBalances
...
setWalletUtxo utxo = do
    wsUtxo .= utxo
    wsBalances .= utxoToCurrencyAddrCoinMap utxo
...
instance Migrate WalletStorage_v2 where
    ...
    migrate WalletStorage_v1{..} = WalletStorage_v2
        ...
        , _v2_wsBalances        = utxoToCurrencyAddrCoinMap _v1_wsUtxo
