...
import           Pos.Core (Coin, Currency, SoftwareVersion)
...
type NewPaymentMulti =
    "paymentsMulti"
    :> Summary "Create a new payment transaction with multiple currencies."
    :> DCQueryParam "passphrase" CPassPhrase
    :> CCapture "from" CAccountId
    :> Capture "to" (CId Addr)
    :> Capture "currency" Currency
    :> Capture "amount" Coin
    :> DReqBody '[JSON] (Maybe InputSelectionPolicy) -- use default policy
    :> WRes Post CTx
...
data WTxsApiRecord route = WTxsApiRecord
  ...
  , _newPaymentBatchMulti :: route
    :- "payments"
    :> "batchMulti"
    :> Summary "Create a new payment transaction with multiple currencies \
               \(can send to multiple recipients)."
    :> DCQueryParam "passphrase" CPassPhrase
    :> ReqBody '[JSON] NewBatchPaymentMulti
    :> WRes Post CTx

  , _newCert :: route
    :- "cert"
    :> Summary "Create a new transaction with cert"
    :> DCQueryParam "passphrase" CPassPhrase
    :> CCapture "from" CAccountId
    :> ReqBody '[JSON] NewCert
    :> WRes Post CTx
