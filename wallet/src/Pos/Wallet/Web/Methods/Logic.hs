...
import qualified Data.List.NonEmpty as NE hiding (map, filter, length, reverse)
...
import           Pos.Client.KeyStorage (MonadKeys (..), MonadKeysRead, addSecretKey,
                                        deleteSecretKeyBy)
import           Pos.Core (Address, mkCoin, sealCurrency, unsafeAddCoin)
...
import           Pos.Txp (GenericTxpLocalData, MonadTxpMem, TxAux, TxId, UndoMap,
                          CurrencyCoinMap, getLocalTxs, getLocalUndos, withTxpLocalData, 
                          applyUtxoModToCurrencyAddrCoinMapMap, getBalanceFromCurrencyAddrCoinMapMap)
...
import           Pos.Wallet.Web.ClientTypes (AccountId (..), CAccount (..), CAccountInit (..),
                                             ...
                                             CWalletMeta (..), Wal, mkCurrencyAmount, encToCId)
...
getAccountMod ws accMod accId = do
    ...
    let allAddrIds = gatherAddresses (camAddresses accMod) dbAddrs
        ...
        allCurrencyAmounts = foldl1 (<>) $ map cadAmount allAddrs  -- NonEmpty CurrencyAmount

    currencyBalances <- mapM decodeCTypeOrFail (NE.toList allCurrencyAmounts) -- [(Currency, Coin)]
    let mergedCurrencyBalances = HM.fromListWith unsafeAddCoin currencyBalances -- HashMap Currency Coin
    let balance = NE.fromList $ map mkCurrencyAmount $ HM.toList mergedCurrencyBalances
...
getWalletIncludeUnready ws mps includeUnready cAddr = do
  ...
  where
    computeBalance accMod = do
        ...
        let addrBalances = getBalancesWithModMulti ws accMod addrIds
        let currencyBalances = gatherAddressBalances addrBalances
        pure $ NE.fromList $ map mkCurrencyAmount $ HM.toList currencyBalances

    noWallet = RequestError $
        sformat ("getWalletIncludeUnready: No wallet with address "%build%" found") cAddr

gatherAddressBalances :: [CurrencyCoinMap]
                      -> CurrencyCoinMap
gatherAddressBalances addrBalances = 
    foldl' gatherAddressBalance (HM.singleton sealCurrency (mkCoin 0)) addrBalances

gatherAddressBalance :: CurrencyCoinMap
                     -> CurrencyCoinMap
                     -> CurrencyCoinMap
gatherAddressBalance currencyCoinMap addrBalance = 
    foldl' (flip $ uncurry $ HM.insertWith unsafeAddCoin) currencyCoinMap $ HM.toList addrBalance
...
getBalanceWithModMulti :: WalletSnapshot -> CachedCAccModifier -> Address -> CurrencyCoinMap
getBalanceWithModMulti ws accMod addr = 
    case getBalancesWithModMulti ws accMod [addr] of 
        []    -> HM.singleton sealCurrency (mkCoin 0)
        (x:_) -> x
...
-- for multi currency
getBalancesWithModMulti :: WalletSnapshot -> CachedCAccModifier -> [Address] -> [CurrencyCoinMap]
getBalancesWithModMulti ws accMod addrs =
        map getBalance addrs
    where
        (walletBalances, utxo) = getWalletBalancesAndUtxo ws
        modifier = camUtxo accMod

        newWalletBalances = applyUtxoModToCurrencyAddrCoinMapMap modifier utxo walletBalances
        getBalance addr = getBalanceFromCurrencyAddrCoinMapMap addr newWalletBalances
...
getWAddressBalanceWithMod ws accMod addr =
    getBalanceWithModMulti ws accMod (view wamAddress addr)
...
getWAddress ws cachedAccModifier cAddr =
    let addr = view wamAddress cAddr
        ...
        balances = getWAddressBalanceWithMod ws cachedAccModifier cAddr
    ...
    in  CAddress aId (NE.fromList $ map mkCurrencyAmount (HM.toList balances)) isUsed isChange
