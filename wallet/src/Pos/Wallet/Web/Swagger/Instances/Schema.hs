...
import           Pos.Core (ApplicationName, BlockCount (..), BlockVersion, ChainDifficulty, Coin,
                           Currency, SlotCount (..), SoftwareVersion, mkCoin, mkCurrency)
...
instance ToParamSchema CT.CCurrency
...
instance ToSchema      CT.CCurrency
instance ToSchema      CT.CurrencyAmount
...
instance ToSchema CT.NewBatchPaymentMulti where
    declareNamedSchema _ = do
        cAccountIdSchema <- declareSchemaRef (Proxy @CT.CAccountId)
        return $ NamedSchema (Just "NewBatchPaymentMulti") $
            sketchSchema example_
                & properties . ix "npbmFrom" .~ cAccountIdSchema
      where
        example_ = CT.NewBatchPaymentMulti
            { CT.npbmFrom = CT.CAccountId "<walletId@accountId>"
            , CT.npbmTo   = (CT.CId (CT.CHash "<address>"), mkCurrency 0, mkCoin 228) :|
                          [(CT.CId (CT.CHash "<address>"), mkCurrency 1, mkCoin 701)]
            , CT.npbmInputSelectionPolicy = OptimizeForSecurity
            }
