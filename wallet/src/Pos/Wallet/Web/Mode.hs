...
import           Data.List.NonEmpty as NE hiding (partition)
...
import           Pos.Core (Address, Coin, Currency, HasConfiguration, HasPrimaryKey (..), isRedeemAddress,
...
import           Pos.Txp (HasTxpConfiguration, MempoolExt, MonadTxpLocal (..), MonadTxpMem, Utxo,
                          addrBelongsToSet, applyUtxoModToCurrencyAddrCoinMapMap, 
                          getBalanceFromCurrencyAddrCoinMapMap, getUtxoModifier,
                          withTxpLocalData)
...
getBalanceDefault :: BalancesEnv ext ctx m => Address -> m (NonEmpty (Currency, Coin))
getBalanceDefault addr = do
    ws <- askWalletSnapshot
    updates <- withTxpLocalData getUtxoModifier
    let (walletBalances, utxo) = getWalletBalancesAndUtxo ws
    return $ NE.fromList $ HM.toList $
        getBalanceFromCurrencyAddrCoinMapMap addr $
        applyUtxoModToCurrencyAddrCoinMapMap updates utxo walletBalances
