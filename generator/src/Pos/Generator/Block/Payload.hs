...
import           Pos.Core (AddrSpendingData (..), Address (..), Coin, SlotId (..), addressHash,
                           coinToInteger, makePubKeyAddressBoot, unsafeIntegerToCoin, mkCoin, sealCurrency)
import           Pos.Core.Txp (Tx (..), TxAux (..), TxIn (..), TxOut (..), pattern SealTxOut, TxOutAux (..))
  ...
  where
    ...
    genTransaction = do
        ...
        let ownUtxo = Utxo.filterUtxoByAddrs inputAddrs utxo
            ...
            -- use seal currency only, for convenient
            totalOwnMoney = coinToInteger $ HM.lookupDefault (mkCoin 0) sealCurrency $ HM.fromList $ NE.toList $ Utxo.getTotalCoinsInUtxo ownUtxo
        ...
        let txOuts :: NonEmpty TxOut
            txOuts = NE.fromList $ zipWith SealTxOut outputAddrs coins
